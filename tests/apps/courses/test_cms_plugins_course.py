...
class CoursePluginTestCase(TestCase):
    ...
    def test_cms_plugins_course_render(self):
        ...
        course_draft = draft.extended_object
        CourseRunFactory.create_batch(2, course=draft)
        ...
        add_plugin(placeholder, CoursePlugin, "fr", **{"course": draft})
        add_plugin(placeholder, CoursePlugin, "en", **{"course": draft})
        ...
        self.assertNotIn("Boring course", response)

        # The course's url should be present
        self.assertContains(
            response,
            '<a class="course-glimpse--link" href="{url}">'.format(
        ...
        self.assertNotIn(
            '<p class="course-glimpse__content__title">{title}</p>'.format(
                title=course_draft.get_absolute_url()
            ),
            response,
        )

        # The course's cover image should be present with the expected resizing
        self.assertContains(response, ".jpg__336x188_q85_subsampling-2.jpg")
